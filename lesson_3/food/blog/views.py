...
class RegisterNews(View):
    ...
    def post(self, request):
        ...
        if news_form.is_valid(): #обязательное поле!!!!!!!!!!!!
            data = news_form.cleaned_data

            new_author = Author.objects.create(username=data['username'], last_name=data['last_name'], first_name=data['first_name'], email=data['email'])
            new_author.save()

            new_news = News.objects.create(title=data['title'], content=data['content'], activity_flag='i', author=new_author)
            new_news.save()
